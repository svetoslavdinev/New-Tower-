...
const T = {
  ...
  ru: {
    ...
    standard: "СТАНДАРТНАЯ", aggressive: "ПРЕДОПЛАТА", deferred: "ГИБКАЯ",
    upToNext50: "до следующих 50", tagHalted: "ПРИОСТАНОВЛЕНО", tagSold: "ПРОДАНО", tagReserved: "ЗАБРОНИРОВАНО", tagGarage: "ГАРАЖ", advBest: "САМАЯ НИЗКАЯ ЦЕНА", advOnly30: "СЕЙЧАС ТОЛЬКО 30%", advBestNote: "Максимальная выгода при полном обязательстве: 80% при предварительном договоре, остальные 20% на Акт 15.", advDeferNote: "Остальные 70% подлежат оплате только на Акт 15 — авг. 2029, через три года. Цена фиксируется сегодня.", advKeep: "Остаётся у вас до 2029:", schemeAdjLbl: "Рассрочка", akt15Note3y: "Взнос 70% подлежит оплате на Акт 15 — авг. 2029, через 3 года после предварительного договора.", earlyCash8020: "80% при предварительном договоре (авг. 2026), остаток 20% на Акт 15 (авг. 2029).", standardName: "Стандартная схема", aggressiveName: "Схема предоплаты", deferredName: "Гибкая схема", youSave: "Вы экономите", deferSurcharge: "Наценка за рассрочку", priceOnRequest: "Цена недоступна для забронированных объектов", inSelection: "В подборке", addToSelection: "Добавить в подборку", mySelection: "Моя подборка", inquiryTitle: "Отправить запрос", selectedProps: "Выбранные объекты", fName: "Имя и фамилия *", fPhone: "Телефон *", fEmail: "Эл. почта", fBudget: "Бюджет (напр. до €250 000)", fComment: "Комментарий / вопрос", schemeStd: "Стандартная схема", sendInquiry: "Отправить запрос", inquiryNote: "Мы свяжемся с вами в течение 24 часов.",
...
const fmtDate = (yyyymm, lang) => {
  const [y, m] = yyyymm.split("-").map(Number);
  return (MONTHS[lang] || MONTHS.bg)[m - 1] + " " + y;
};
// нива на подземния паркинг
const LVL = {
  bg: { "подз.-1": "подз.-1", "подз.-2": "подз.-2" },
  en: { "подз.-1": "Level -1", "подз.-2": "Level -2" },
  ru: { "подз.-1": "подз.-1", "подз.-2": "подз.-2" },
};
const tLvl = (v, lang) => ((LVL[lang] || LVL.bg)[v] || v);
// ценова група на ПМ (данните са на български)
const tGroup = (g, lang) => {
  if (!g) return g;
  if (g.indexOf("ФИКС") === 0) return (lang === "en" ? "FIXED" : "ФИКС") + g.slice(4);
  if (g === "ПАКЕТ") return lang === "en" ? "PACKAGE" : "ПАКЕТ";
  return g;
};
...
const getScheme = (schemeId, floor, L) => {
  ...
  // C · 30/70 — разсрочено плащане
  if (schemeId === "deferred") {
    return {
      id: "deferred",
      label: L.deferredName,
      subtitle: "30 / 70",
      discountPct: 0,
      m2Adjust: 200,
      steps: [
...
export default function App() {
  ...
  // цена на избата към даден апартамент (за списъка на обектите)
  const izbaPriceOf = (a) => {
    if (!a || !a.iz) return 0;
    const s = INV.S.find((x) => x.id === a.iz);
    return s && s.p ? s.p : 0;
  };
      ...
      <div style={{ maxWidth: 1250, margin: "0 auto", padding: mob ? "14px 10px" : "24px 20px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
            {/* COUNTER — до следващи 50 */}
            {(() => {
              const left = Math.max(0, 50 - [...INV.A, ...INV.B].filter((a) => a.rez).length);
              return (
                <div style={{
                  display: "flex", alignItems: "center", gap: 8,
                  padding: mob ? "5px 10px" : "6px 14px", borderRadius: 20,
                  background: C.gold + "18", border: "1px solid " + C.gold + "55",
                }}>
                  <span style={{ fontSize: mob ? 16 : 20, fontWeight: 900, lineHeight: 1, color: C.gold, ...mono }}>{left}</span>
                  <span style={{ fontSize: mob ? 8 : 9, color: C.gr, letterSpacing: 0.5, lineHeight: 1.2 }}>{L.upToNext50}</span>
                </div>
              );
            })()}
            {/* Lang switcher */}
                      ...
                      {apts.map((a) => {
                        ...
                        return (
                          <tr key={a.s + "-" + a.n} onClick={() => { if (!isLocked) setSelected(a); }} style={{
                            ...
                            <td style={{ padding: "8px 6px", textAlign: "right", color: isLocked ? C.grD : C.gold, fontWeight: 800, ...mono, textDecoration: isRez ? "line-through" : "none" }}>{isSold ? "—" : fmt(a.calc.price + izbaPriceOf(a))}</td>
                          </tr>
                        );
                      })}
                    </tbody>
                  ...
                  {sel.rez && (
                    <div style={{
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 10, color: C.gr, marginTop: 2 }}>{L.priceOnRequest}</div>
                      </div>
                    </div>
                  )}
                    ...
                    {showParkingPicker && (
                      <div style={{ marginTop: 10, maxHeight: 250, overflowY: "auto",
                        ...
                        {INV.P.map((p) => {
                          ...
                          const tag = p.sold ? (p.reason === "stop" ? L.tagHalted : L.tagSold) : p.rez ? L.tagReserved : null;
                          return (
                          <button key={p.id} disabled={taken}
                            ...
                            <span style={{ color: C.gr, fontSize: 9 }}>{tLvl(p.lvl, lang)}</span>
                            {tag ? (
                              <span style={{ color: p.rez ? C.gold : C.err, fontSize: 8, fontWeight: 700, letterSpacing: 0.3 }}>{tag}</span>
                            ) : (
                              <span style={{ color: p.g === "TBD" ? C.err : p.g === "ПАКЕТ" ? C.pur : C.gr, fontSize: 9 }}>{p.isGarage ? L.tagGarage : tGroup(p.g, lang)}</span>
                            )}
                ...
                <div style={card}>
                  ...
                  <div style={{
                    ...
                    {[
                      ...
                      { id: "deferred", label: L.deferred, sub: "30 / 70" },
                    ].map((s) => (
                          ...
                          <div style={{ flex: 1, marginLeft: 6 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 4 }}>
                              ...
                              <div style={{ textAlign: "right" }}>
                                ...
                                <div style={{ fontSize: 9, color: C.grD }}>{fmtDate(s.when, lang)}</div>
                              </div>
                            </div>
                      ...
                      <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                        ...
                        <tbody>
                          {[
                            ...
                            { l: L.parkingCap, f: (sc) => sc.parking ? `${sc.parking.id} (${sc.parking.p ? fmt(sc.parking.p) : L.onRequest})` : "—" },
              ...
              <div id="nt-offer-print" style={{
                ...
                <div className="nt-o-header" style={{
                  ...
                  <div style={{ textAlign: "right", flexShrink: 0 }}>
                    <div style={{ fontSize: 8, color: "#E8C061", letterSpacing: 1 }}>{L.ofNo}</div>
                ...
                <div className="nt-o-section" style={{ padding: "15px 30px" }}>
                  <div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 700, borderBottom: "1px solid #E8C061", paddingBottom: 4, marginBottom: 10 }}>{L.ofApt}</div>
                  <div className="nt-o-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px 30px", fontSize: 11 }}>
                    <div className="nt-o-row">
                      <span className="lbl">{L.ofLocation}</span>
                      <span className="val">Сек.{sel.s} · Ап.{sel.n} · Ет.{sel.f}</span>
                    </div>
                    <div className="nt-o-row">
                      <span className="lbl">{L.ofNet}</span>
                ...
                {/* PACKAGE */}
                <div className="nt-o-section" style={{ padding: "15px 30px" }}>
                  <div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 700, borderBottom: "1px solid #E8C061", paddingBottom: 4, marginBottom: 10 }}>{L.pkg}</div>
                    ...
                    <tbody>
                      ...
                      {activeIzba && (
                        <tr><td style={{ padding: "8px 5px", borderBottom: "1px solid #F3F4F6" }}>
                          ...
                          <div style={{ fontSize: 10, color: "#6B7280" }}>{tLvl("подз.-1", lang)} · {activeIzba.r.toFixed(2)} {L.m2} · €1,500/{L.m2}{sel.pmDoor ? " · 🚪 " + sel.pmDoor : ""}</div>
                ...
                <div className="nt-o-total" style={{
                  ...
                  <span className="nt-o-total-lbl" style={{ fontWeight: 700, fontSize: 12, letterSpacing: 1 }}>{L.ofTotal}</span>
                ...
                <div className="nt-o-section" style={{ padding: "15px 30px" }}>
                  ...
                  <div style={{
                    ...
                    <div>
                      <div style={{ fontWeight: 800, fontSize: 11 }}>{L.ofResFee}</div>
                      <div style={{ fontSize: 9, color: "#6B7280", marginTop: 2 }}>{L.ofResFeeNote}</div>
                    </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 10 }}>
                    <thead>
                      <tr style={{ background: "#1A1F3D", color: "white" }}>
                        ...
                        <th style={{ padding: "7px 8px", textAlign: "left", fontSize: 9, letterSpacing: 1 }}>{L.ofStep}</th>
                        <th style={{ padding: "7px 8px", textAlign: "left", fontSize: 9, letterSpacing: 1 }}>{L.ofDesc}</th>
                        <th style={{ padding: "7px 8px", textAlign: "left", fontSize: 9, letterSpacing: 1 }}>{L.ofDue}</th>
                        <th style={{ padding: "7px 8px", textAlign: "right", fontSize: 9, letterSpacing: 1 }}>%</th>
                        <th style={{ padding: "7px 8px", textAlign: "right", fontSize: 9, letterSpacing: 1 }}>{L.ofSum}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {paymentSteps.map((s, i) => (
                        <tr key={i} style={{ background: i % 2 === 0 ? "#F9FAFB" : "white" }}>
                          ...
                          <td style={{ padding: "8px", color: "#6B7280" }}>{s.when ? fmtDate(s.when, lang) : ""}</td>
                      ...
                      <tr style={{ background: "#E8C061", color: "#1A1F3D", fontWeight: 800 }}>
                        ...
                        <td colSpan="3" style={{ padding: "10px 8px" }}>{L.ofTotNoFee}</td>
                ...
                <div className="nt-o-section" style={{ padding: "0 30px 15px 30px" }}>
                  <div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 700, borderBottom: "1px solid #E8C061", paddingBottom: 4, marginBottom: 8 }}>{L.ofTerms}</div>
                  <ul style={{ listStyle: "none", padding: 0, margin: 0, fontSize: 10, color: "#374151" }}>
                    <li style={{ padding: "3px 0" }}><span style={{ color: "#E8C061", fontWeight: 800 }}>• </span>{L.ofT1}</li>
                    <li style={{ padding: "3px 0" }}><span style={{ color: "#E8C061", fontWeight: 800 }}>• </span>{L.ofT2}</li>
                    <li style={{ padding: "3px 0" }}><span style={{ color: "#E8C061", fontWeight: 800 }}>• </span>{L.ofT3}</li>
                    <li style={{ padding: "3px 0" }}><span style={{ color: "#E8C061", fontWeight: 800 }}>• </span>{L.ofT4}</li>
                ...
                <div className="nt-o-footer" style={{ background: "#F5E6C8", padding: "15px 30px", display: "flex", justifyContent: "space-between" }}>
                  <div>
                    <div style={{ fontSize: 10, letterSpacing: 1, fontWeight: 700 }}>{L.ofBroker}</div>
                    <div style={{ fontWeight: 800, fontSize: 13, marginTop: 4 }}>{L.ofBrokerPh}</div>
                    <div style={{ fontSize: 10, color: "#374151", marginTop: 3 }}>{L.ofCompany}</div>
                    <div style={{ fontSize: 10, color: "#374151", marginTop: 3 }}>{L.ofContact}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: 800, fontSize: 11 }}>NEW TOWER</div>
                    <div style={{ fontSize: 10, color: "#374151", marginTop: 3 }}>{L.ofAddr}</div>
